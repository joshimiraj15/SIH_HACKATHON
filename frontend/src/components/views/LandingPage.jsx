...
const GLOBAL_CSS = `
  @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=Playfair+Display:ital,wght@0,600;0,700;0,800;0,900;1,400&display=swap');

  .fb-root * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'DM Sans', sans-serif; }
  .fb-root { background: ${T.cream}; color: ${T.charcoal}; overflow-x: hidden; }

  /* Headings with elegant serif */
  .fb-root h1, .fb-root h2, .fb-root h3, .fb-root .fb-serif {
  ...
  .fb-sparkle {
    ...
    color: ${T.maroon}; opacity: 0.22;
    animation: fb-float 6s ease-in-out infinite;
  }
  .fb-sparkle-green { color: ${T.green}; opacity: 0.45; }
  @keyframes fb-float {
  ...
  /* Pulse dot */
  @keyframes fb-pulse {
    0%,100% { box-shadow: 0 0 0 3px rgba(53,94,59,.22); }
    50%      { box-shadow: 0 0 0 7px rgba(53,94,59,.06); }
  }
  .fb-tag-dot {
    width: 8px; height: 8px; border-radius: 50%; background: ${T.green};
    box-shadow: 0 0 0 3px rgba(53,94,59,.22);
  ...
  .fb-btn-dark { background: ${T.maroon}; color: #fff; }
  .fb-btn-dark:hover { background: #561221; transform: translateY(-1px); box-shadow: 0 8px 28px rgba(112,27,46,.25); }
  .fb-btn-green { background: ${T.green}; color: #fff; }
  .fb-btn-green:hover { background: #27472D; transform: translateY(-1px); box-shadow: 0 8px 28px rgba(53,94,59,.25); }
  .fb-btn-outline { background: transparent; color: ${T.maroon}; border: 1.5px solid ${T.maroon}; }
  .fb-btn-outline:hover { background: ${T.maroon}; color: #fff; transform: translateY(-1px); }

  /* Navbar */
  .fb-nav { position: sticky; top: 0; z-index: 100; background: rgba(250,246,240,.94); backdrop-filter: blur(16px); border-bottom: 1px solid ${T.border}; }
  ...
  .fb-nav-logo { display: flex; align-items: center; gap: 10px; font-size: 20px; font-weight: 800; font-family: 'Playfair Display', serif; letter-spacing: -.3px; color: ${T.maroon}; text-decoration: none; cursor: pointer; background: none; border: none; }
  .fb-nav-logo-icon { width: 34px; height: 34px; background: ${T.maroon}; border-radius: 8px; display: grid; place-items: center; flex-shrink: 0; }
  .fb-nav-links { display: flex; align-items: center; gap: 32px; list-style: none; }
  .fb-nav-links a { font-size: 13.5px; font-weight: 600; color: ${T.gray}; text-decoration: none; transition: color .2s; cursor: pointer; }
  .fb-nav-links a:hover { color: ${T.maroon}; }
  .fb-nav-signin { font-size: 13.5px; font-weight: 600; color: ${T.maroon}; text-decoration: none; padding: 8px 14px; transition: color .2s; cursor: pointer; background: none; border: none; font-family: inherit; }
  ...
  .fb-hero-tag { display: inline-flex; align-items: center; gap: 9px; font-size: 11.5px; font-weight: 700; letter-spacing: .09em; text-transform: uppercase; color: ${T.green}; margin-bottom: 32px; background: ${T.sage}; padding: 6px 14px; border-radius: 999px; border: 1px solid ${T.border}; }
  .fb-hero-headline { font-family: 'Playfair Display', Georgia, serif; font-size: clamp(38px, 3.8vw, 64px); font-weight: 900; letter-spacing: -1.5px; line-height: 1.05; color: ${T.maroon}; margin-bottom: 26px; }
  ...
  .fb-hero-img-wrap { position: relative; border-radius: 20px; overflow: hidden; aspect-ratio: 3/4; max-height: 570px; box-shadow: 0 24px 80px rgba(112,27,46,.12); }
  ...
  .fb-hero-badge { position: absolute; bottom: 24px; left: 24px; background: rgba(250,246,240,.96); backdrop-filter: blur(10px); border-radius: 14px; padding: 14px 18px; display: flex; align-items: center; gap: 12px; box-shadow: 0 4px 20px rgba(44,30,26,.1); border: 1px solid ${T.border}; }
  .fb-badge-dot { width: 10px; height: 10px; border-radius: 50%; background: ${T.green}; box-shadow: 0 0 0 3px rgba(53,94,59,.22); animation: fb-pulse 2.2s ease-in-out infinite; flex-shrink: 0; }
  ...
  .fb-full-img-wrap { position: relative; border-radius: 22px; overflow: hidden; aspect-ratio: 16/6.8; border: 1px solid ${T.border}; }
  ...
  .fb-full-img-overlay { position: absolute; inset: 0; background: linear-gradient(to top, rgba(86,18,33,.85) 0%, rgba(86,18,33,.25) 45%, transparent 70%); }
  ...
  .fb-full-tagline { font-family: 'Playfair Display', serif; font-size: clamp(20px, 2.4vw, 32px); font-weight: 800; letter-spacing: -.3px; color: #fff; max-width: 420px; line-height: 1.25; }
  .fb-full-demo { display: inline-flex; align-items: center; gap: 9px; font-size: 11.5px; font-weight: 600; color: rgba(255,255,255,.9); letter-spacing: .07em; text-transform: uppercase; border: 1px solid rgba(255,255,255,.38); padding: 11px 20px; border-radius: 999px; cursor: pointer; transition: all .22s; background: rgba(112,27,46,.4); backdrop-filter: blur(4px); font-family: inherit; }
  .fb-full-demo:hover { background: rgba(255,255,255,.2); color: #fff; }

  /* Stats */
  #fb-stats { border-top: 1px solid ${T.border}; border-bottom: 1px solid ${T.border}; margin-top: 80px; background: ${T.sage}; }
  ...
  .fb-stat:hover { background: rgba(53,94,59,.06); }
  .fb-stat-num { font-family: 'Playfair Display', serif; font-size: clamp(32px, 3.2vw, 50px); font-weight: 900; letter-spacing: -1.2px; color: ${T.maroon}; margin-bottom: 10px; }
  .fb-stat-num span { color: ${T.green}; }
  .fb-stat-lbl { font-size: 12px; font-weight: 700; color: ${T.gray}; letter-spacing: .05em; text-transform: uppercase; }
  ...
  .fb-problem-h { font-family: 'Playfair Display', serif; font-size: clamp(28px, 3.2vw, 52px); font-weight: 900; letter-spacing: -1px; line-height: 1.1; color: ${T.maroon}; margin-bottom: 52px; }
  .fb-tags { display: flex; gap: 12px; flex-wrap: wrap; }
  .fb-tag { padding: 10px 24px; border-radius: 999px; border: 1.5px solid ${T.border}; font-size: 13px; font-weight: 600; color: ${T.charcoal}; background: #fff; transition: all .22s; cursor: default; }
  .fb-tag:hover { border-color: ${T.green}; color: ${T.green}; background: ${T.sage}; }
  ...
  .fb-hiw-h { font-family: 'Playfair Display', serif; font-size: clamp(26px, 2.8vw, 42px); font-weight: 900; letter-spacing: -.5px; color: ${T.maroon}; margin-bottom: 60px; max-width: 460px; }
  ...
  .fb-step:hover { background: ${T.sage}; }
  ...
  .fb-step-icon { width: 46px; height: 46px; background: ${T.maroon}; border-radius: 12px; display: grid; place-items: center; margin-bottom: 20px; transition: background .25s; }
  ...
  .fb-step-title { font-family: 'Playfair Display', serif; font-size: 17px; font-weight: 700; color: ${T.charcoal}; margin-bottom: 12px; letter-spacing: -.2px; }
  ...
  .fb-feat-h { font-family: 'Playfair Display', serif; font-size: clamp(26px, 2.8vw, 42px); font-weight: 900; letter-spacing: -.5px; color: ${T.maroon}; max-width: 400px; }
  .fb-feat-grid { display: grid; grid-template-columns: repeat(3, 1fr); border: 1px solid ${T.border}; border-radius: 20px; overflow: hidden; background: #fff; }
  ...
  .fb-feat-card:hover { background: ${T.sage}; }
  .fb-feat-icon { width: 48px; height: 48px; border: 1.5px solid ${T.border}; border-radius: 12px; display: grid; place-items: center; margin-bottom: 24px; transition: all .3s; background: ${T.cream}; }
  .fb-feat-card:hover .fb-feat-icon { background: ${T.green}; border-color: ${T.green}; }
  .fb-feat-icon svg { width: 22px; height: 22px; stroke: ${T.maroon}; fill: none; stroke-width: 1.8; stroke-linecap: round; stroke-linejoin: round; transition: stroke .3s; }
  .fb-feat-card:hover .fb-feat-icon svg { stroke: #fff; }
  .fb-feat-title { font-family: 'Playfair Display', serif; font-size: 17px; font-weight: 700; color: ${T.charcoal}; margin-bottom: 12px; letter-spacing: -.2px; }
  .fb-feat-desc { font-size: 13px; color: ${T.gray}; line-height: 1.78; }
  .fb-feat-corner { position: absolute; bottom: 18px; right: 22px; font-size: 24px; opacity: .1; color: ${T.maroon}; transition: opacity .3s, color .3s; pointer-events: none; }
  .fb-feat-card:hover .fb-feat-corner { opacity: .25; color: ${T.green}; }

  /* Footer CTA - Deep Burgundy Banner */
  #fb-footer-cta { background: ${T.maroon}; padding: 110px 0; position: relative; overflow: hidden; }
  .fb-cta-glow { position: absolute; inset: 0; background: radial-gradient(ellipse at 60% 50%, rgba(53,94,59,.35) 0%, transparent 68%); pointer-events: none; }
  .fb-cta-inner { text-align: center; position: relative; z-index: 1; }
  .fb-cta-label { font-size: 11px; font-weight: 700; letter-spacing: .15em; text-transform: uppercase; color: rgba(255,255,255,.6); margin-bottom: 28px; }
  .fb-cta-h { font-family: 'Playfair Display', serif; font-size: clamp(38px, 5.5vw, 68px); font-weight: 900; letter-spacing: -1.5px; color: #fff; line-height: 1.05; margin-bottom: 52px; }
  .fb-cta-h em { font-style: normal; color: #F5B038; }
  .fb-cta-actions { display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; }
  .fb-btn-white { background: #fff; color: ${T.maroon}; padding: 15px 34px; border-radius: 999px; font-size: 15px; font-weight: 700; cursor: pointer; border: none; font-family: inherit; transition: all .25s; box-shadow: 0 4px 16px rgba(0,0,0,.15); }
  .fb-btn-white:hover { background: ${T.cream}; transform: translateY(-2px); box-shadow: 0 10px 32px rgba(0,0,0,.25); }
  .fb-btn-ghost { background: transparent; color: rgba(255,255,255,.85); padding: 15px 34px; border-radius: 999px; font-size: 15px; font-weight: 600; cursor: pointer; border: 1.5px solid rgba(255,255,255,.35); font-family: inherit; transition: all .25s; }
  .fb-btn-ghost:hover { border-color: #fff; color: #fff; background: rgba(255,255,255,.1); }

  /* Footer */
  .fb-footer { background: #3B1019; padding: 44px 0; border-top: 1px solid rgba(255,255,255,.08); }
  ...
  .fb-footer-logo { font-family: 'Playfair Display', serif; font-size: 16px; font-weight: 800; color: rgba(255,255,255,.7); letter-spacing: -.2px; }
  .fb-footer-links { display: flex; gap: 28px; }
  .fb-footer-links a { font-size: 12px; color: rgba(255,255,255,.5); text-decoration: none; transition: color .2s; }
  .fb-footer-links a:hover { color: #fff; }
  .fb-footer-copy { font-size: 12px; color: rgba(255,255,255,.35); }
  ...
  .fb-mq-item { flex-shrink: 0; width: 300px; height: 220px; border-radius: 16px; overflow: hidden; position: relative; border: 1px solid ${T.border}; }
  ...
  .fb-mq-label { position: absolute; bottom: 0; left: 0; right: 0; padding: 10px 14px; background: linear-gradient(to top, rgba(86,18,33,.7), transparent); font-size: 11px; font-weight: 600; letter-spacing: .06em; text-transform: uppercase; color: rgba(255,255,255,.9); opacity: 0; transition: opacity .3s; }
        ...
        <div className="fb-nav-inner">
          ...
          <div className="fb-nav-actions">
            {isLoggedIn ? (
              ...
              <>
                <button className="fb-btn fb-btn-green" style={{ background: '#355E3B', color: '#FFFFFF' }} onClick={() => onExploreApp('home')}>Explore Dashboard →</button>
