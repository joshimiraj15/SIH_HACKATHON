...
const AIChatbot = ({ language, setLanguage, setActiveTab }) => {
  ...
  // Helper for Text-to-Speech (AI voice response)
  ...
  const handleSendMessage = async (textToSend) => {
    ...
    try {
      ...
      // Add a medium slow delay (1.5 seconds) to make it feel natural
      await new Promise(resolve => setTimeout(resolve, 1500));

      if (geminiReply && geminiReply.trim()) {
        const finalReply = geminiReply.trim();
        setMessages((prev) => [
          ...prev,
          {
            id: Date.now() + 1,
      ...
      // Add a medium slow delay (1.5 seconds) even on error
      await new Promise(resolve => setTimeout(resolve, 1500));
  ...
  const handleVoiceInput = () => {
    ...
    if (!SpeechRecognition) {
      alert(language === 'gu' ? 'તમારું બ્રાઉઝર વોઇસ રેકગ્નિશન સપોર્ટ કરતું નથી.' : language === 'hi' ? 'आपका ब्राउज़र वॉयस रिकग्निशन को सपोर्ट नहीं करता है।' : 'Your browser does not support Voice Recognition.');
      return;
    }
    ...
    recognition.onresult = (event) => {
      ...
      if (finalTranscript) {
        ...
        // Automatically send the spoken text and stop immediately to prevent duplicates
      ...
      {isOpen && (
        <div className="ai-chat-window">
          ...
          >
            <input 
              type="text" 
              placeholder={isListening ? (language === 'gu' ? 'સાંભળી રહ્યા છીએ...' : language === 'hi' ? 'सुन रहा हूँ...' : 'Listening...') : t.typeQuestion} 
              value={inputMessage}
            ...
            <button 
              ...
              disabled={isTyping}
              title={isListening ? 'Recording...' : 'Voice Typing'}
            >
